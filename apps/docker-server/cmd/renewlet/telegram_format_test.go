...
func TestTelegramFormatterEscapesHTMLMode(t *testing.T) {
	...
	if !strings.Contains(notification.Text, `<b>Renewo &amp; &lt;Friends&gt;</b>`) || strings.Contains(notification.Text, "<Friends>") {
		t.Fatalf("notification html was not escaped: %q", notification.Text)
	}

	bot := formatTelegramBotMessage("Renewo status\nTotal: 12\n总数：12\nA&B <Pro>", telegramMessageFormatHTML)
	if bot.ParseMode != "HTML" {
		t.Fatalf("expected bot HTML parse mode, got %#v", bot)
	}
	if !strings.Contains(bot.Text, "<b>Renewo status</b>") || !strings.Contains(bot.Text, "Total: <b>12</b>") || !strings.Contains(bot.Text, "总数：<b>12</b>") {
