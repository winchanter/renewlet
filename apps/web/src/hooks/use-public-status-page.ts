...
export function useUpdatePublicStatusPage() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (body: { showPrices: boolean; vaultEnabled: boolean }) => publicStatusService.updatePage(body),
    onSuccess: (page) => {
      queryClient.setQueryData(PUBLIC_STATUS_PAGE_QUERY_KEY, page);
    },
  });
...
export function useDeletePublicStatusPage() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      queryClient.setQueryData(PUBLIC_STATUS_PAGE_QUERY_KEY, { enabled: false, showPrices: false, vaultEnabled: false });
    },
  });
}
